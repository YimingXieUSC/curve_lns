                                                                                                   ...
                                                                                                   path_table(instance.map_size), pipp_option(pipp_option)
{
    ...
    else if (destory_name == "GridCollision")
    {
        destroy_strategy = GRIDCOLLISION;
        cout << "The destroy_strategy is grid collision" << endl;
    }
    else
    {
...
bool LNS::run()
{
    ...
    if (screen == -1)
    {
        cout << "STATS BEFORE WHILE LOOP: "
             << "\nRuntime: " << runtime << ", time_limit: " << time_limit << ", iteration size: " << iteration_stats.size() << ", num_of_iterations: " << num_of_iterations << endl;
    }
    ...
    while (runtime < time_limit && iteration_stats.size() <= num_of_iterations)
    {
        ...
        case RANDOMAGENTS:
            ...
            break;
        case GRIDCOLLISION:
            succ = generateNeighborByGridCollision();
            break;
        default:
...
struct CompareCollision
{
    bool operator()(CollisionInfo const &c1, CollisionInfo const &c2)
    {
...
bool LNS::generateNeighborByGridCollision()
{
    ...
    // agentGrid and counterGrid initializations
    int maxTimestep = 0;
    for (const auto &a : agents)
    {
        if (a.path.size() > maxTimestep)
        {
            maxTimestep = a.path.size();
        }
    }
    cout << "Max timestep: " << maxTimestep << endl;

    vector<vector<vector<Agent>>> agentGrid(instance.map_size, vector<vector<Agent>>(maxTimestep)); // space-time grid consists a vector of agent with collision
    //vector<vector<int> > counterGrid(instance.map_size, vector<CollisionInfo>(maxTimestep, {0,0,0})); // space-time grid consist total number of agents
    vector<vector<CollisionInfo>> counterGrid;

    for (int i = 0; i < instance.map_size; i++)
    {
        vector<CollisionInfo> tmpvector;
        for (int j = 0; j < maxTimestep; j++)
        {
            tmpvector.push_back({i, j, 0});
        }
        counterGrid.push_back(tmpvector);
    }
    cout << "agentgrid size: " << agentGrid.size() << ", grid width: " << agentGrid[0].size() << endl;
    cout << "counter grid size: " << counterGrid.size() << ", grid width: " << counterGrid[0].size() << endl;
    for (const auto &a1_ : agents)
    {
        for (const auto &a2_ : agents)
        {
            ...
            {
                if (a1.path[t].location == a2.path[t].location) // vertex conflict detected
                {
                    if (screen == -1)
                    {
                        cerr << "GRID DETECTOR: VERTEXT CONFLICT FOUND" << endl;
                    }
                    cout << "location: " << a1.path[t].location << ", t: " << t << endl;
                    // if a vertext conflict exits, put both agents causing the conflict into
                    // the agentGrid at that space-time
                    // counterGrid[a1.path[t].location][t] += 2;
                    counterGrid[a1.path[t].location][t].counter += 2;
                    // // TODO: check whether agentGrid already contains the agent at that space-time
                    // agentGrid[a1.path[t].location][t].push_back(a1);
                    // agentGrid[a1.path[t].location][t].push_back(a2);

                    cerr << "VERTEXT CONFLICT RECORDED" << endl;
                }
                else if (a1.path[t].location == a2.path[t - 1].location &&
                         a1.path[t - 1].location == a2.path[t].location) // edge conflict detected
                {
                    if (screen == -1)
                    {
                        cerr << "GRID DETECTOR: EDGE CONFLICT FOUND" << endl;
                    }
                    // if an edge conflict detected, put both agents causing the edge conflict
                    // into the agent at that space(two locations)-time
                    // counterGrid[a1.path[t-1].location][t] += 2;
                    // counterGrid[a1.path[t].location][t] += 2;
                    counterGrid[a1.path[t - 1].location][t].counter += 2;
                    counterGrid[a1.path[t].location][t].counter += 2;
                    agentGrid[a1.path[t].location][t].push_back(a1);
                    agentGrid[a1.path[t].location][t].push_back(a2);
                    agentGrid[a1.path[t - 1].location][t].push_back(a1);
                    agentGrid[a1.path[t - 1].location][t].push_back(a2);
                }
            }
            ...
            for (; t < (int)a2.path.size(); t++)
            {
                ...
                if (a2.path[t].location == target) // target conflict detected
                {
                    if (screen == -1)
                    {
                        cerr << "TARGET CONFLICT FOUND" << endl;
                    }

                    // counterGrid[a2.path[t].location][t] += 1;
                    counterGrid[a2.path[t].location][t].counter += 1;
                    agentGrid[a2.path[t].location][t].push_back(a2);
                }
            }
        }
    }
    ...
    // ##################################################
    // cerr << "FINISH" << endl;
    // cerr << "neighbor_size" << neighbor_size << endl;

    // for(int i = 0; i < instance.map_size; i++){
    //     for(int j = 0; j < maxTimestep; j++){
    //         cerr << counterGrid[i][j] << " ";
    //     }
    //     cerr << endl;
    // }
    // ##################################################

    // choose the top location with the most conflicts
    //int counter = 0;
    // while(counter < neighbor_size){

    // }

    // todo: assign newly chosed neighbors back and return bool
    cout << "neighbor size" << neighbor_size << endl;

    set<int> neighbors_set;
    std::priority_queue<CollisionInfo, vector<CollisionInfo>, CompareCollision> collision_rank;
    ...
    {
        if (!collision_rank.empty())
        {
            CollisionInfo ci = collision_rank.top();
            if (ci.counter != 0)
            {
                cout << "collision info: counter: " << ci.counter << endl;
                // todo: optimization
                vector<Agent> collidingAgents = agentGrid[ci.location][ci.time];
                for(int j = 0; j < collidingAgents.size(); j++){
                    neighbors_set.insert(collidingAgents[j].id);
                }
            }
            else{
                // no collision in leftover positions
                break;
            }
            collision_rank.pop();
        }
    ...
    cout << "happen at the end of the function" << endl;
    neighbor.agents.assign(neighbors_set.begin(), neighbors_set.end());

    cout << "All selected neighbors" << endl;
